...
# 创建根证书请求文件
openssl req -new -key root.key -out root.csr -subj "/emailAddress=liujinpu@bjtxra.com/CN=liujinpu/OU=devbj/O=TXRA/L=BJ/ST=BJ/C=CN"
# 创建根证书
openssl x509 -req -in root.csr -out root.crt -signkey root.key -CAcreateserial -days 3650 
...
# 生成客户端证书请求文件
openssl req -new -key client.key -out client.csr -subj "/emailAddress=liujinpu@bjtxra.com/CN=liujinpu/OU=devbj/O=TXRA/L=BJ/ST=BJ/C=CN"
# 生成客户端证书
openssl x509 -req -in client.csr -out client.crt -signkey client.key -CA root.crt -CAkey root.key -CAcreateserial -days 3650
# 生客户端p12格式证书，需要输入一个密码，选一个好记的，比如123456
